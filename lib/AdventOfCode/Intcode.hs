...
memRead addr Computer { romem = ro, rwmem = rw } =
  fromMaybe (if inBounds then ro ! addr else 0) (IntMap.lookup addr rw)
  where (l, h) = bounds ro
        inBounds = l <= addr && addr <= h
...
step' op modes comp =
  ...
  Jump p -> let (test:posIfTrue:_) = resolved
    in computer' { pos = if p test then posIfTrue else pos' }
